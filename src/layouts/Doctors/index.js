...
function Doctors() {
    ...
    if (!token) window.location.href = "/authentication/sign-in";
      ...
      if (payload) {
        const res = await axios.post(
          endPoint + "/users/booking_request",
          payload,
        );
        if (res.data.success) {
  ...
  const getLabs = async () => {
    setLoading(true);
    const posts = await axios.get(endPoint + "/users/doctors");
  ...
  return isAuth ? (
    <React.Fragment>
      ...
      >
        ...
        <div className="input2" style={{ marginTop: 25}}>
        <textarea
            style={{ padding: 25}}
            value={description}
            onChange={(e) => {
            ...
            type="text"
            required="required"
            placeholder="Enter Details/Symptoms"
          />
          </div>
        <Button
          disabled={description.length === 0 || requestedDate === ""}
          variant="contained"
          color="success"
